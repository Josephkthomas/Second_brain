...
export default function FirefliesSetup({
  ...
  useEffect(() => {
    if (isConnected && userIntegration?.webhook_token) {
      setWebhookUrl(buildWebhookUrl('fireflies', userIntegration.webhook_token));
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        const items = (data.items || []).filter(
          (item: any) => item.metadata?.ingestion_method === 'fireflies_webhook'
        );
  ...
  const handleGenerateUrl = async () => {
    ...
    try {
      const { data, error } = await createUserIntegration(
        'fireflies',
        { ...integration.defaultConfig, api_key: apiKey },
        true
      );
      if (data?.webhook_token) {
        setWebhookUrl(buildWebhookUrl('fireflies', data.webhook_token));
        setStep(3);
      } else {
  ...
  const handleRegenerate = async () => {
    ...
    if (token) setWebhookUrl(buildWebhookUrl('fireflies', token));
    setRegenerating(false);
  };

  // ── CONNECTED STATE ────────────────────────────────────────
  if (isConnected && step === 4 && userIntegration) {
        ...
        <div>
          ...
          {activityItems.length === 0 && !activityLoading ? (
            <div className="p-4 bg-slate-800/50 border border-white/5 rounded-xl text-center">
              ...
              <p className="text-[10px] text-slate-500 mt-1">Wait for your next meeting to finish recording in Fireflies.</p>
            </div>
          ) : (
  ...
  if (step === 1) {
    return (
      <div className="space-y-5">
        ...
        <div>
          <h3 className="text-sm font-semibold text-white mb-1">Enter your Fireflies API key</h3>
          <p className="text-xs text-slate-400 leading-relaxed">
            Synapse needs your API key to fetch full transcripts from Fireflies after
            each meeting. It's stored securely and never shared.
          </p>
          <div className="mt-3 p-3 bg-amber-900/20 border border-amber-500/30 rounded-lg">
            <p className="text-xs text-amber-400">
              Requires a Fireflies Business plan. Free and Pro plans do not include API access.
            </p>
          </div>
        </div>
        <input
          ...
          className="w-full px-3 py-2.5 bg-slate-800 border border-white/10 rounded-lg text-sm text-white placeholder-slate-500 focus:outline-none focus:border-cyan-500/50"
        />
        <p className="text-xs text-slate-500">
          Find it in Fireflies &rarr; Settings &rarr; Integrations &rarr; Developer
        </p>
        <button
          onClick={() => apiKey.trim() && setStep(2)}
          disabled={!apiKey.trim()}
          className="w-full py-2.5 bg-cyan-600 hover:bg-cyan-500 disabled:opacity-40 text-white text-sm font-medium rounded-lg transition-colors"
        >
          Continue &rarr;
        </button>
      </div>
    ...
    return (
      <div className="space-y-5">
        ...
        <div>
          ...
          <p className="text-xs text-slate-400 leading-relaxed">
            Synapse will create a unique URL linked to your API key. When Fireflies
            notifies this URL, Synapse fetches the full transcript automatically.
          </p>
        </div>
  ...
  // ── STEP 3: Configure in Fireflies ─────────────────────────
  if (step === 3) {
    return (
      <div className="space-y-5">
        <StepIndicator current={3} total={4} />
        <div>
          <h3 className="text-sm font-semibold text-white mb-1">Add this URL to Fireflies</h3>
          <p className="text-xs text-slate-400 leading-relaxed">
            Copy your webhook URL below, then follow these steps in Fireflies:
          </p>
        </div>
        ...
        <ol className="space-y-2.5">
          {[
            'Open Fireflies and go to Settings → Integrations → Developer',
            'Scroll to the Webhooks section',
            'Paste your webhook URL and click Save',
            'Fireflies will notify Synapse when each meeting transcript is ready',
          ].map((instruction, i) => (
        ...
        <a href="https://app.fireflies.ai/settings" target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-xs text-red-400 hover:text-red-300 transition-colors">
          <ExternalLink size={12} /> Open Fireflies Settings
        </a>
        <button onClick={() => setStep(4)} className="w-full py-2.5 px-4 bg-slate-700 hover:bg-slate-600 text-white text-sm font-medium rounded-lg transition-colors">
          I've set it up in Fireflies &rarr;
        </button>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <div>
        ...
        <p className="text-xs text-slate-400 leading-relaxed">
          Record a meeting with Fireflies. Once the transcription completes,
          Fireflies will fire the webhook and Synapse will fetch the full transcript.
        </p>
      </div>
      <div className="p-4 bg-slate-800/50 border border-white/5 rounded-xl space-y-2">
        ...
        {[
          'Fireflies notifies Synapse when a transcription completes',
          'Synapse fetches the full transcript via your API key',
