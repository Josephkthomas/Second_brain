...
export default function MeetgeekSetup({
  integration,
  userIntegration,
  onSuccess,
  onDisconnect,
  ...
  useEffect(() => {
    if (isConnected && userIntegration?.webhook_token) {
      setWebhookUrl(buildWebhookUrl('meetgeek', userIntegration.webhook_token));
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        const items = (data.items || []).filter(
          (item: any) => item.metadata?.ingestion_method === 'meetgeek_webhook'
        );
  ...
  const handleGenerateUrl = async () => {
    ...
    try {
      ...
      if (data?.webhook_token) {
        setWebhookUrl(buildWebhookUrl('meetgeek', data.webhook_token));
        setStep(2);
      } else {
  ...
  const handleRegenerate = async () => {
    ...
    if (token) setWebhookUrl(buildWebhookUrl('meetgeek', token));
        ...
        <div>
          ...
          {activityItems.length === 0 && !activityLoading ? (
            <div className="p-4 bg-slate-800/50 border border-white/5 rounded-xl text-center">
              ...
              <p className="text-[10px] text-slate-500 mt-1">Wait for your next meeting to finish in MeetGeek.</p>
            </div>
          ) : (
      ...
      <div className="space-y-5">
        ...
        <div className="p-4 bg-slate-800/50 border border-white/5 rounded-xl space-y-2.5">
          ...
          {[
            ...
            'Add the URL to MeetGeek Integrations',
      ...
      <div className="space-y-5">
        ...
        <div>
          ...
          <p className="text-xs text-slate-400 leading-relaxed">
            Synapse will create a unique URL for your account. Any meeting MeetGeek sends to this URL
  ...
  // ── STEP 2: Configure in MeetGeek ────────────────────────────
  if (step === 2) {
    return (
      <div className="space-y-5">
        <StepIndicator current={2} total={3} />
        <div>
          <h3 className="text-sm font-semibold text-white mb-1">Add this URL to MeetGeek</h3>
          <p className="text-xs text-slate-400 leading-relaxed">
            Copy your webhook URL below, then follow these steps in MeetGeek:
          </p>
        </div>
        ...
        <ol className="space-y-2.5">
          {[
            'Open MeetGeek and go to Integrations → Public API',
            'Scroll to the Webhooks section',
            'Enter your webhook URL and click Save',
            'MeetGeek will send meeting data automatically after each meeting completes',
          ].map((instruction, i) => (
        ...
        <a href="https://app.meetgeek.ai/integrations" target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-xs text-emerald-400 hover:text-emerald-300 transition-colors">
          <ExternalLink size={12} /> Open MeetGeek Integrations
        </a>
        <button onClick={() => setStep(3)} className="w-full py-2.5 px-4 bg-slate-700 hover:bg-slate-600 text-white text-sm font-medium rounded-lg transition-colors">
          I've set it up in MeetGeek &rarr;
        </button>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <div>
        ...
        <p className="text-xs text-slate-400 leading-relaxed">
          Join or record a meeting with MeetGeek. Once processing completes,
          MeetGeek will fire the webhook and your tile will turn green.
        </p>
      </div>
      <div className="p-4 bg-slate-800/50 border border-white/5 rounded-xl space-y-2">
        ...
        {[
          'MeetGeek fires the webhook after each meeting is processed',
          'Synapse receives the summary, transcript & action items',
