...
export const UserProfileContent: React.FC = () => {
  ...
  const handleBlur = useCallback((
    ...
    if (!profile) return;
    handleSave({ [section]: { ...(profile[section] || {}), [field]: value } });
  }, [profile, handleSave]);
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-4">
      ...
      {/* Professional Context */}
      <Section title="Professional Context" icon={<Briefcase size={18} />} defaultOpen={true}>
        <div className="space-y-4">
          <div>
            <label className="block text-xs text-slate-400 mb-1.5">Your Role</label>
            <input type="text" placeholder="e.g., Product Manager, Software Engineer, Researcher" className="w-full bg-cyber-slate/50 border border-white/10 rounded-lg px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-cyber-cyan/50 transition-colors" value={profile?.professional_context?.role || ''} onChange={(e) => updateField('professional_context', 'role', e.target.value)} onBlur={(e) => handleBlur('professional_context', 'role', e.target.value)} maxLength={100} />
          </div>
          <div>
            <label className="block text-xs text-slate-400 mb-1.5">Industry / Domain</label>
            <input type="text" placeholder="e.g., AI/ML, Healthcare, Finance, Education" className="w-full bg-cyber-slate/50 border border-white/10 rounded-lg px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-cyber-cyan/50 transition-colors" value={profile?.professional_context?.industry || ''} onChange={(e) => updateField('professional_context', 'industry', e.target.value)} onBlur={(e) => handleBlur('professional_context', 'industry', e.target.value)} maxLength={100} />
          </div>
          <div>
            <label className="block text-xs text-slate-400 mb-1.5">Current Projects / Focus Areas</label>
            <textarea placeholder="e.g., Building a knowledge graph tool, researching RAG systems, launching a new product" className="w-full bg-cyber-slate/50 border border-white/10 rounded-lg px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-cyber-cyan/50 transition-colors resize-none h-24" value={profile?.professional_context?.current_projects || ''} onChange={(e) => updateField('professional_context', 'current_projects', e.target.value)} onBlur={(e) => handleBlur('professional_context', 'current_projects', e.target.value)} maxLength={500} />
          </div>
        </div>
      </Section>

      {/* Personal Interests */}
      <Section title="Personal Interests" icon={<User size={18} />} defaultOpen={true}>
        <div className="space-y-4">
          <div>
            <label className="block text-xs text-slate-400 mb-1.5">Topics You Care About</label>
            <textarea placeholder="e.g., AI, cognitive science, personal productivity, philosophy, economics" className="w-full bg-cyber-slate/50 border border-white/10 rounded-lg px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-cyber-cyan/50 transition-colors resize-none h-24" value={profile?.personal_interests?.topics || ''} onChange={(e) => updateField('personal_interests', 'topics', e.target.value)} onBlur={(e) => handleBlur('personal_interests', 'topics', e.target.value)} maxLength={500} />
          </div>
          <div>
            <label className="block text-xs text-slate-400 mb-1.5">Learning Goals</label>
            <textarea placeholder="e.g., Understanding graph databases, improving critical thinking, learning about quantum computing" className="w-full bg-cyber-slate/50 border border-white/10 rounded-lg px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-cyber-cyan/50 transition-colors resize-none h-24" value={profile?.personal_interests?.learning_goals || ''} onChange={(e) => updateField('personal_interests', 'learning_goals', e.target.value)} onBlur={(e) => handleBlur('personal_interests', 'learning_goals', e.target.value)} maxLength={500} />
          </div>
        </div>
      </Section>

      {/* Processing Preferences */}
      <Section title="Processing Preferences" icon={<Sparkles size={18} />} defaultOpen={true}>
        <div className="space-y-4">
          <div>
            <label className="block text-xs text-slate-400 mb-1.5">Insight Depth</label>
            <select className="w-full bg-cyber-slate/50 border border-white/10 rounded-lg px-4 py-2.5 text-sm text-white focus:outline-none focus:border-cyber-cyan/50 transition-colors appearance-none cursor-pointer" value={profile?.processing_preferences?.insight_depth || ''} onChange={(e) => { updateField('processing_preferences', 'insight_depth', e.target.value); handleBlur('processing_preferences', 'insight_depth', e.target.value); }}>
              <option value="" className="bg-cyber-slate">No preference</option>
              <option value="detailed" className="bg-cyber-slate">Detailed - Extract everything relevant</option>
              <option value="high-level" className="bg-cyber-slate">High-level - Key concepts only</option>
            </select>
          ...
          <div>
            <label className="block text-xs text-slate-400 mb-1.5">Relationship Focus</label>
            <select className="w-full bg-cyber-slate/50 border border-white/10 rounded-lg px-4 py-2.5 text-sm text-white focus:outline-none focus:border-cyber-cyan/50 transition-colors appearance-none cursor-pointer" value={profile?.processing_preferences?.relationship_focus || ''} onChange={(e) => { updateField('processing_preferences', 'relationship_focus', e.target.value); handleBlur('processing_preferences', 'relationship_focus', e.target.value); }}>
              <option value="" className="bg-cyber-slate">No preference</option>
              <option value="broad" className="bg-cyber-slate">Broad - Many connections across topics</option>
              <option value="deep" className="bg-cyber-slate">Deep - Fewer, stronger connections</option>
            </select>
      ...
      <div className="text-center pt-4 pb-8">
        <p className="text-xs text-slate-500">Changes are saved automatically when you leave each field.</p>
      </div>
    </div>
  );
};
