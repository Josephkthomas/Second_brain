// Shared delivery utilities for digest email/telegram/slack
// Used by both cron.ts (auto-delivery) and deliver.ts (manual delivery)
// Prefixed with _ to exclude from Vercel routing

// ─── Email Formatter ────────────────────────────────────────

export function formatDigestEmail(content: any, profileName: string): { subject: string; html: string } {
  ...
  const stepsHtml = (content.suggested_next_steps || []).map((step: any) => {
    ...
    return `
    <tr>
      <td style="padding: 6px 0; vertical-align: top; width: 55px;">
        <span style="display: inline-block; padding: 2px 7px; border-radius: 4px; font-family: ${font}; font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; color: ${ps.text}; background: ${ps.bg}; border: 1px solid ${ps.border};">${step.priority}</span>
      </td>
      <td style="padding: 6px 0; padding-left: 6px;">
        <span style="color: #f1f5f9; font-family: ${font}; font-size: 13px; font-weight: 500; display: block;">${step.action}</span>
        <span style="color: #64748b; font-family: ${font}; font-size: 11px; display: block; margin-top: 2px;">${step.rationale}</span>
      </td>
    </tr>`;
  }).join('');
...
<body style="margin: 0; padding: 0; background: #020617; font-family: ${font};">
  <div style="max-width: 580px; margin: 0 auto; padding: 32px 16px;">
    ...
    <div style="margin-bottom: 24px; border-radius: 8px; overflow: hidden; border: 1px solid rgba(251,191,36,0.15);">
      <div style="padding: 12px 16px; background: #111827; border-bottom: 1px solid rgba(251,191,36,0.1);">
        <span style="color: #fbbf24; font-family: ${font}; font-size: 14px; font-weight: 600;">Suggested Next Steps</span>
      </div>
      <div style="padding: 12px 16px; background: #0a0f1a;">
...
export function formatDigestTelegram(content: any, profileName: string): string {
  const date = new Date(content.generated_at).toLocaleDateString('en-GB', {
...
export async function sendTelegram(botToken: string, chatId: string, text: string): Promise<{ success: boolean; error?: string }> {
  try {
...
export function formatDigestSlack(content: any, profileName: string): any {
  const date = new Date(content.generated_at).toLocaleDateString('en-GB', {
    weekday: 'long', day: 'numeric', month: 'long',
  });
...
export async function sendSlack(webhookUrl: string, payload: any): Promise<{ success: boolean; error?: string }> {
  try {
...
// ─── Channel Delivery Orchestrator ───────────────────────────
// Used by cron.ts for automatic delivery after generation
...
export async function deliverToChannels(
    ...
    try {
      if (channel.channel_type === 'email') {
        ...
        if (success) {
          ...
          errors.email = error || 'Send failed';
        }
      }
      ...
      if (channel.channel_type === 'telegram') {
        ...
        if (!botToken || !chatId) {
          errors.telegram = 'Bot token or chat ID not configured';
          continue;
        }
        const text = formatDigestTelegram(digestContent, profileName);
        ...
        if (success) {
          ...
          errors.telegram = error || 'Send failed';
        }
      }
      ...
      if (channel.channel_type === 'slack') {
        ...
        if (!webhookUrl) {
          errors.slack = 'Webhook URL not configured';
          continue;
        }
        const payload = formatDigestSlack(digestContent, profileName);
        const { success, error } = await sendSlack(webhookUrl, payload);
        if (success) {
          if (!deliveredChannels.includes('slack')) deliveredChannels.push('slack');
        } else {
          errors.slack = error || 'Send failed';
        }
      }
    } catch (err) {
  ...
  // Update channels_delivered in history if any channel was delivered
  if (deliveredChannels.length > 1) {
    await supabase
      .from('digest_history')
      .update({ channels_delivered: deliveredChannels })
      .eq('id', historyId);
  }
